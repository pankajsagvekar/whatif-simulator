...
exports.SeriousOutcomeGenerator = void 0;
...
class SeriousOutcomeGenerator {
     ...
     */
    async generateSeriousOutcome(scenario) {
        const context = `serious outcome for "${scenario.originalText}"`;
        try {
            return await ErrorHandler_1.ErrorHandler.withRetry(async () => {
                const prompt = this.buildSeriousPrompt(scenario);
                const response = await this.aiService.generateResponse(prompt);
                // Validate response quality
                if (!ErrorHandler_1.ErrorHandler.validateContent(response)) {
                    throw new ErrorHandler_1.WhatIfSimulatorError('AI response was too short or invalid', ErrorHandler_1.ErrorType.AI_GENERATION, undefined, true);
                }
            ...
            if (error instanceof ErrorHandler_1.WhatIfSimulatorError && !error.retryable) {
                return ErrorHandler_1.ErrorHandler.createFallbackContent('serious', scenario.originalText, error.message);
            }
            ...
            return ErrorHandler_1.ErrorHandler.createFallbackContent('serious', scenario.originalText, fallbackReason);
        }
    }
    /**
     ...
     */
    formatSeriousResponse(response, scenario) {
     ...
     */
    generateContextualNote(scenario) {
...
exports.SeriousOutcomeGenerator = SeriousOutcomeGenerator;
//# sourceMappingURL=SeriousOutcomeGenerator.js.map