import { AppConfig, DEFAULT_APP_CONFIG, createAppConfigFromEnvironment } from './WhatIfSimulatorApp';
...
export function getConfigForEnvironment(env?: string): AppConfig {
  ...
  switch (environment) {
    case 'production':
      baseConfig = CONFIG_PROFILES.production;
      break;
    case 'test':
      baseConfig = CONFIG_PROFILES.test;
      break;
    case 'development':
    default:
      baseConfig = CONFIG_PROFILES.development;
      break;
  }
  
  // Override with environment variables
  const envConfig = createAppConfigFromEnvironment();
