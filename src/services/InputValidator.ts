...
export class InputValidator {
   ...
   */
  public validateScenario(input: string): ValidationResult {
    try {
      // Handle null/undefined input
      if (!input) {
        return this.createValidationError(
          '',
          'Please provide a "What if..." question to explore.'
        );
      }
      ...
      try {
        ...
        ErrorHandler.logError(error as Error, 'input sanitization');
        return this.createValidationError(
          input,
          'Unable to process input. Please try rephrasing your scenario.'
        );
      }
      ...
      if (sanitizedInput.trim().length < this.minLength) {
        return this.createValidationError(
          sanitizedInput,
          'Please provide a more detailed "What if..." scenario (at least 10 characters).'
        );
      }
      ...
      if (sanitizedInput.length > this.maxLength) {
        return this.createValidationError(
          sanitizedInput.substring(0, this.maxLength),
          'Scenario is too long. Please keep it under 1000 characters.'
        );
      }

      // Check for inappropriate content with error handling
      let inappropriateCheck: { isAppropriate: boolean; reason?: string };
      try {
        inappropriateCheck = this.checkInappropriateContent(sanitizedInput);
      } catch (error) {
        ErrorHandler.logError(error as Error, 'inappropriate content check');
        // If content checking fails, err on the side of caution
        return this.createValidationError(
          sanitizedInput,
          'Unable to verify content appropriateness. Please try rephrasing your scenario.'
        );
      }
      ...
      if (!inappropriateCheck.isAppropriate) {
        return this.createValidationError(
          sanitizedInput,
          'Please rephrase your scenario to avoid inappropriate content.'
        );
      }

      // Check if it's a meaningful scenario with error handling
      let isMeaningful: boolean;
      try {
        isMeaningful = this.isMeaningfulScenario(sanitizedInput);
      } catch (error) {
        ErrorHandler.logError(error as Error, 'meaningful scenario check');
        // If meaningfulness check fails, provide helpful guidance
        return this.createValidationError(
          sanitizedInput,
          'Please provide a more specific "What if..." scenario. Try starting with "What if..." followed by a clear situation.'
        );
      }
      ...
      if (!isMeaningful) {
        return this.createValidationError(
          sanitizedInput,
          'Please provide a more specific "What if..." scenario that can be analyzed.'
        );
      }
      ...
      return this.createValidationError(
        input || '',
        'An error occurred while validating your input. Please try again with a different scenario.'
      );
    }
  }
   ...
   */
  private createValidationError(sanitizedInput: string, errorMessage: string): ValidationResult {
    return {
      isValid: false,
      sanitizedInput,
      errorMessage
   ...
   */
  private checkInappropriateContent(input: string): { isAppropriate: boolean; reason?: string } {
   ...
   */
  private isMeaningfulScenario(input: string): boolean {
