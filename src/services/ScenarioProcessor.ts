...
export class ScenarioProcessor {
    ...
    try {
      ...
      if (!scenario || typeof scenario !== 'string' || scenario.trim().length === 0) {
        throw new WhatIfSimulatorError(
          'Invalid scenario input provided',
      ...
      if (!scenarioType || !keyElements || !complexity) {
        throw new WhatIfSimulatorError(
          'Failed to properly analyze scenario structure',
   ...
   */
  private createFallbackProcessedScenario(scenario: string): ProcessedScenario {
   ...
   */
  private identifyScenarioType(scenario: string): ProcessedScenario['scenarioType'] {
   ...
   */
  private extractKeyElements(scenario: string): ProcessedScenario['keyElements'] {
   ...
   */
  private extractActors(scenario: string): string[] {
    const actors: string[] = [];
    const lowerScenario = scenario.toLowerCase();
   ...
   */
  private extractActions(scenario: string): string[] {
   ...
   */
  private extractContext(scenario: string): string {
   ...
   */
  private assessComplexity(scenario: string, keyElements: ProcessedScenario['keyElements']): ProcessedScenario['complexity'] {
    let complexityScore = 0;

    // Length factor
    const wordCount = scenario.split(/\s+/).filter(word => word.length > 0).length;
    if (wordCount > 25) complexityScore += 3;
    else if (wordCount > 15) complexityScore += 2;
    else if (wordCount > 8) complexityScore += 1;

    // Number of actors
    if (keyElements.actors.length > 3) complexityScore += 2;
    else if (keyElements.actors.length > 1) complexityScore += 1;

    // Number of actions
    if (keyElements.actions.length > 3) complexityScore += 2;
    else if (keyElements.actions.length > 1) complexityScore += 1;
    ...
    const complexKeywordCount = complexKeywords.filter(keyword => 
      lowerScenario.includes(keyword)
    ).length;
    ...
    const moderateKeywordCount = moderateKeywords.filter(keyword => 
      lowerScenario.includes(keyword)
    ).length;
